...
# USIM Training Script for User Simulator with Slime Backend
# Trains user simulator on tau2-bench tasks using Qwen3-4B
...
OUTPUT_DIR="${OUTPUT_DIR:-/scratch/usim_slime/$(date +%Y%m%d_%H%M%S)}"
...
CKPT_ARGS=(
   --hf-checkpoint "${WORKSPACE_DIR}/Qwen3-4B-Base"
   --ref-load "${WORKSPACE_DIR}/Qwen3-4B-Base_torch_dist"
...
WANDB_ARGS=(
   --use-wandb
   --wandb-project usim
   --wandb-group qwen3-4B-usim-slime
...
# USIM Evaluation Config
# Evaluates user simulator on held-out tau2-bench tasks
