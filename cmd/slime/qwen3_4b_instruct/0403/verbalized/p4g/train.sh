...
# Date: 2026-04-08
#
# Verbalized Sampling: the persuadee is prompted to generate N candidate
# replies with verbalized probabilities per turn, then one is sampled. The
# instruction preserves the [DONATE $N] / [GIVE $N] markers so the P4G
# reward extraction still fires on sampled responses.
...
OUTPUT_DIR="${OUTPUT_DIR:-/scratch/usim_slime/0403_p4g_verbalized_gemini/$(date +%Y%m%d_%H%M%S)}"
...
CKPT_ARGS=(
   ...
   --save "${OUTPUT_DIR}/Qwen3-4B-Instruct-2507_verbalized_p4g/"
   --save-interval 16
)
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3-4B-Instruct-2507-p4g-verbalized-gemini-0403
