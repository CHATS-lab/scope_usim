...
# Date: 2026-04-08
#
# Verbalized Sampling: the user simulator is prompted to generate N candidate
# responses with verbalized probabilities per turn, then one is sampled. This
# mitigates mode collapse in the instruction-tuned user sim.
...
OUTPUT_DIR="${OUTPUT_DIR:-/scratch/usim_slime/0403_tau2_verbalized_gemini/$(date +%Y%m%d_%H%M%S)}"
...
CKPT_ARGS=(
   ...
   --save "${OUTPUT_DIR}/Qwen3-4B-Instruct-2507_verbalized_tau2/"
   --save-interval 16
)
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3-4B-Instruct-2507-tau2-verbalized-gemini-0403
