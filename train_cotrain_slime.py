...
def add_cotrain_arguments(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    mode_group.add_argument(
        "--tau2-user-reward-mode",
        type=str,
        default="cooperative",
        choices=["cooperative", "curriculum"],
        help=(
            "How to compute the user simulator's reward on τ²-bench co-training:\n"
            "  cooperative = same reward as agent (default, matches prior runs)\n"
            "  curriculum  = SPICE-style Gaussian on per-prompt agent-reward variance,\n"
            "                r = exp(-(var - 0.25)^2 / 0.02), peaks at 50/50 pass rate."
        ),
    )

    mode_group.add_argument(
        "--tau2-tool-error-penalty",
        type=float,
        default=-0.1,
        help=(
            "Penalty added to the user's curriculum reward when an episode "
            "ends in a τ² tool error. Only applies when "
