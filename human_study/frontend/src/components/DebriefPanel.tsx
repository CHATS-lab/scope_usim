...
interface Props {
  ...
  prolificRedirectUrl: string | null;
  debrief: DebriefInfo;
}
...
export function DebriefPanel({
  ...
  // 8-second auto-redirect so participants don't need to copy anything.
  // Counts down visibly so they can cancel if they want to read the debrief first.
  const [secondsLeft, setSecondsLeft] = useState(8);

  // The code participants paste back on Prolific: the Prolific-issued code
  // when available, NOT our internal HMAC. completionCode is kept around only
  // as a fallback if the Prolific code wasn't configured (older deployments).
  const displayCode = prolificCode || completionCode;

  useEffect(() => {
    if (!prolificRedirectUrl) return;
    if (secondsLeft <= 0) {
  ...
  }, [secondsLeft, prolificRedirectUrl]);
      ...
      <article className="w-full max-w-xl space-y-6 rounded-xl border border-border bg-panel p-8 shadow-xl">
        ...
        >
          ...
          <p className="mt-1 text-xs text-muted">
            {prolificRedirectUrl
              ? `You'll be redirected back to Prolific automatically in ${secondsLeft}s. If the redirect doesn't fire, click the button below or paste this code on the Prolific submission page.`
              : "Paste this code into the Prolific submission page to mark the task as completed. Copy it before closing this tab."}
          </p>
          ...
          {prolificRedirectUrl && (
            <a
              ...
              className="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-accent/50 bg-accent/10 px-3 py-2 text-sm font-medium text-text hover:bg-accent/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
            >
