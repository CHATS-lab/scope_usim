...
def start_session(
    ...
    if existing:
        ...
        base = dict(
            ...
            task_structured=task.get("structured"),
            variant=existing.variant,
            resumed=True,
        )
        if existing.status == SessionStatus.SURVEY_DONE:
            # Re-surface the completion code + debrief instead of rejecting —
            # a participant re-opening the link shouldn't hit a dead end. Also
            # include the Prolific code + redirect URL so the debrief panel
            # auto-redirects to Prolific rather than showing the internal HMAC.
            from ..config import settings as _s
            pcc = {
                "tau2": _s.prolific_completion_code_tau2,
                "p4g": _s.prolific_completion_code_p4g,
            }.get(existing.task_type.value, "") or None
