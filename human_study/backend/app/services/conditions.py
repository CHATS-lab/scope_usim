...
def model_for_condition(condition: Condition) -> tuple[str, str | None]:
    """Return (model_name, base_url_or_None) for an OpenAI-compatible client."""
    match condition:
        case Condition.BASE:
            return settings.model_base, settings.openai_base_url_base or None
        case Condition.RL_SINGLE:
            return settings.model_rl_single, settings.openai_base_url_rl_single or None
        case Condition.COTRAINING:
            return settings.model_cotraining, settings.openai_base_url_cotraining or None
