...
def dual_cotrain(args):
    ...
    pg, pg_bundle_indices, pg_gpu_ids = _create_placement_group(total_gpus)
    init_tracking(args)
    # Opponent's args was deep-copied before init_tracking, so it's missing the
    # wandb_run_id that init_wandb_secondary needs to attach opponent's rank-0
    # process to the same W&B run. Without this, opponent rank 0 skips
    # wandb.init() but still calls wandb.log() in train_actor → crash.
    opponent_args.wandb_run_id = getattr(args, "wandb_run_id", None)
